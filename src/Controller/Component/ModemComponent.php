...
class ModemComponent extends Component {
    ...
    {
        $this->host = ! empty($config['host']) ? $config['host'] : '192.168.8.1';
        $this->username = 'admin';
     ...
     * LocalDraft
     */
    public function smscount($boit = 'LocalInbox') {
        if (!$this->islogged())
            $this->login();
        ...
        $response = new \SimpleXMLElement($res);
        return $response->$boit;
    }

    // type : 1 = inbox, 2 = sent
    public function smsread($type = 1, $page = 1) {
        if (!$this->islogged()) $this->login();

        $req = new \SimpleXMLElement('<request></request>');
        $req->addChild('PageIndex', $page);
        $req->addChild('ReadCount', 10);
